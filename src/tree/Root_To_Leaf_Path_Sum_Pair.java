...
public class Root_To_Leaf_Path_Sum_Pair {
    ...
    public static void main(String...strings){
        ...
        int sum = node1.data;
        boolean printPairs = true;
        isPairPresent(node1, sum, printPairs);
    }

    private static void isPairPresent(Node root, int target, boolean printPairs) {
        Set<Integer> hashSet = new HashSet<>();
        if(printPairs) {
            System.out.println("Pairs : ");
            isPairPresent(root.left, target, hashSet, printPairs);
            isPairPresent(root.right, target, hashSet, printPairs);
        }
        else
            System.out.println(
                    "Is pair present " + 
    ...
    private static boolean isPairPresent(Node node, int target, Set<Integer> hashSet, boolean printPairs) {
        if(node != null) {
            ...
            if(isPairPresent(node.left, target, hashSet, printPairs) || isPairPresent(node.right, target, hashSet, printPairs))
                return true;
            else
